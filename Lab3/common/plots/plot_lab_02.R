...
# plot: time for freivalds: different algorithms along avg and max running time
pdf('p3.pdf', height=4, width=6)
ggplot(data[data$type == 'freivalds',], aes(x = log2(size), y = 1000 * time)) +
    geom_bar(stat = 'identity') +
    facet_grid(measure ~ name, scales = "free", space = "free") +
    xlab('log(Tamaño entrada)') +
    ylab('Tiempo en milisegundos') +
    ggtitle('Freivalds: tiempo') +
    theme(plot.title = element_text(hjust = 0.5)) # center title
dev.off()

# plot: error for sorting: different algorithms along avg and max running time
pdf('p4.pdf', height=3, width=6)
ggplot(data[data$type == 'freivalds' & data$measure == 'avg',], aes(x = log2(size), y = 1 - error)) +
    geom_bar(stat = 'identity') +
    facet_grid(~ name, scales = "free", space = "free") +
    xlab('log(Tamaño entrada)') +
    ylab('1 - %error') +
    ggtitle('Freivalds: error') +
    theme(plot.title = element_text(hjust = 0.5)) # center title
dev.off()

# plot: time for freivalds: different algorithms along avg and max running time
pdf('p5.pdf', height=4, width=4)
ggplot(data[data$type == 'problema-3-8',], aes(x = log2(size), y = 1000 * time)) +
    geom_bar(stat = 'identity') +
    facet_grid(measure ~ name, scales = "free", space = "free") +
    xlab('log(Tamaño entrada)') +
    ylab('Tiempo en milisegundos') +
    ggtitle('Problema 3-8: tiempo') +
    theme(plot.title = element_text(hjust = 0.5)) # center title
dev.off()

# plot: error for sorting: different algorithms along avg and max running time
pdf('p6.pdf', height=4, width=4)
ggplot(data[data$type == 'problema-3-8' & data$measure == 'avg',], aes(x = log2(size), y = 1 - error)) +
    geom_bar(stat = 'identity') +
    facet_grid(~ name, scales = "free", space = "free") +
    xlab('log(Tamaño entrada)') +
    ylab('1 - %error') +
    ggtitle('Problema 3-8: error') +
    theme(plot.title = element_text(hjust = 0.5)) # center title
dev.off()
